...
async function getRemoteSecret(domain, alg, kid, cache) {
  try {
    ...
    const response = await got(`${domain}.well-known/jwks.json`, {
      responseType: 'json',
    });
    ...
    if (e.response) {
      throw InternalServerError(
        `${errorMessages.jwksHttpError}: [HTTP ${
...
function getSecret(request, reply, cb) {
  ...
  getRemoteSecret(
    request.auth0Verify.domain,
    header.alg,
    header.kid,
    request.auth0VerifySecretsCache
  )
...
function fastifyAuth0Verify(instance, options, done) {
  try {
    ...
    const ttl =
      parseFloat(
        'secretsTtl' in options ? options.secretsTtl : '604800000',
        10
      ) / 1e3;
...
module.exports = fastifyPlugin(fastifyAuth0Verify, {
